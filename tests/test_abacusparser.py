...
def test_band(parser):
    ...
    assert sec_k_band_segment.kpoints[3][2] == 0.425
...
def test_scf(parser):
    ...
    assert sec_scc.energy.electrostatic.correction.magnitude == approx(2.402022465119184e-18)
    ...
    assert sec_eigenvalues.energies[0][4][1].magnitude == approx(-2.3289399769487398e-20)
    ...
    assert sec_eigenvalues.kpoints[4][0] == 0.75
...
def test_geomopt(parser):
    ...
    assert sec_workflow.geometry_optimization.convergence_tolerance_force_maximum.magnitude == approx(1.6021766339999997e-12)
    assert sec_workflow.geometry_optimization.x_abacus_geometry_optimization_threshold_stress.magnitude == approx(1000000.0)
...
def test_md(parser):
    ...
    assert sec_sccs[5].energy.electronic.kinetic.magnitude == approx(1.748126841263409e-18)
...
def test_hse(parser):
    ...
    assert sec_scc.energy.hartree_fock_x_scaled.value.magnitude == approx(-1.2417912255666503e-17)
